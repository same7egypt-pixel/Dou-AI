...
def _country_name(code: str) -> str:
    return {"SA": "Saudi Arabia", "EG": "Egypt"}.get(
        (code or "").upper(), (code or "Unknown").upper()
    )


def ensure_geo_country(db: Session, tenant: Tenant) -> GeoCountry:
    code = (
        tenant.market_code or getattr(tenant.country, "value", tenant.country) or "SA"
    ).upper()
...
def ensure_tenant_operating_city(
    db: Session, tenant: Tenant, city: GeoCity, active: bool = True
) -> TenantOperatingCity:
    record = (
    ...
    if record:
        ...
        return record
    record = TenantOperatingCity(
        tenant_id=tenant.id, geo_city_id=city.id, is_active=active
    )
    db.add(record)
    db.flush()
    return record
...
def active_tenant_city_ids(db: Session, tenant_id: int) -> set[int]:
    return {
        row[0]
        for row in db.query(TenantOperatingCity.geo_city_id)
        .filter(
...
def resolve_active_tenant_city_by_name(
    ...
    for link in links:
        ...
        if (
            ...
            and city.active
            and normalized
            in {normalize_city_name(city.name), normalize_city_name(link.display_name)}
        ):
            matches.append(city)
    if len(matches) != 1:
...
def require_branch_assignment(
    ...
    if (
        not contract
        or contract.tenant_id != tenant_id
        or not branch
        or branch.tenant_id != tenant_id
        or branch.contract_id != contract.id
...
def validate_supervisor_for_branch(
    db: Session, tenant_id: int, supervisor_id: Optional[int], branch: ContractBranch
) -> Optional[User]:
...
def backfill_operating_cities(db: Session) -> dict:
    ...
    for tenant in tenants:
        branches = (
            db.query(ContractBranch).filter(ContractBranch.tenant_id == tenant.id).all()
        )
...
def operating_city_counts(db: Session, tenant_id: int, city_id: int) -> dict:
    return {
        "branches": db.query(ContractBranch)
        .filter(
            ContractBranch.tenant_id == tenant_id, ContractBranch.city_id == city_id
        )
        .count(),
        "riders": db.query(Courier)
        .filter(Courier.tenant_id == tenant_id, Courier.city_id == city_id)
        .count(),
        "supervisors": db.query(User)
        .filter(User.tenant_id == tenant_id, User.role == UserRole.SUPERVISOR)
        .filter(
            User.id.in_(
                db.query(ContractBranch.supervisor_id).filter(
                    ContractBranch.tenant_id == tenant_id,
                    ContractBranch.city_id == city_id,
                    ContractBranch.supervisor_id.isnot(None),
                )
            )
        )
        .count(),
        "contracts": db.query(ContractBranch.contract_id)
        .filter(
            ContractBranch.tenant_id == tenant_id, ContractBranch.city_id == city_id
        )
        .distinct()
