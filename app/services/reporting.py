...
def _scoped_courier_query(
    ...
    if user.role == UserRole.SUPERVISOR:
        ...
        try:
            managed = [
                int(value) for value in json.loads(user.managed_project_ids or "[]")
            ]
        except (TypeError, ValueError, json.JSONDecodeError):
            managed = []
        query = (
            query.filter(Courier.primary_project_id.in_(managed))
            if managed
...
def _apply_filters(query, db: Session, tenant_id: int, filters: dict):
    ...
    if filters.get("search"):
        ...
        query = query.filter(
            or_(Courier.name.ilike(needle), Courier.phone.ilike(needle))
        )
    ...
    effective_on = filters.get("effective_on") or date.today()
    if isinstance(effective_on, str):
        effective_on = date.fromisoformat(effective_on)
    selected_team_ids = None
    if filters.get("team_id"):
        selected_team_ids = db.query(WorkforceTeam.id).filter(
            WorkforceTeam.tenant_id == tenant_id,
            WorkforceTeam.id == int(filters["team_id"]),
        )
    elif filters.get("zone_id"):
        selected_team_ids = db.query(WorkforceTeam.id).filter(
            WorkforceTeam.tenant_id == tenant_id,
            WorkforceTeam.zone_id == int(filters["zone_id"]),
        )
    if selected_team_ids is not None:
        member_ids = db.query(TeamMembership.courier_id).filter(
            TeamMembership.tenant_id == tenant_id,
            TeamMembership.team_id.in_(selected_team_ids),
            TeamMembership.effective_from <= effective_on,
            or_(
                TeamMembership.effective_to.is_(None),
                TeamMembership.effective_to >= effective_on,
            ),
        )
        query = query.filter(Courier.id.in_(member_ids))
    if filters.get("employment_status"):
        query = query.filter(
            Courier.employment_status == str(filters["employment_status"]).upper()
        )
...
def _names(db: Session, tenant_id: int, couriers: list[Courier]) -> dict:
    ...
    cities = (
        {
            row.id: row.name
            for row in db.query(GeoCity).filter(GeoCity.id.in_(city_ids)).all()
        }
        if city_ids
        else {}
    )
    contracts = {
        row.id: row
        for row in db.query(Contract).filter(Contract.tenant_id == tenant_id).all()
    }
    return {
        "branches": branches,
        "projects": projects,
...
def _document_status(courier: Courier, today: date) -> str:
    dates = [
        ...
        courier.passport_expiry,
        courier.insurance_expiry,
        courier.inspection_expiry,
...
def _activity_maps(
    ...
    attendance = (
        ...
        .filter(
            ...
            Attendance.check_in
            < datetime.combine(
                end.fromordinal(end.toordinal() + 1), datetime.min.time()
            ),
        )
        .all()
    )
    for row in attendance:
        ...
        if row.check_out:
            result["worked_hours"][row.courier_id] += max(
                (row.check_out - row.check_in).total_seconds() / 3600, 0
            )
        if row.is_late:
...
def _financial_maps(
    db: Session, tenant_id: int, month: str, allowed_ids: set[int]
) -> tuple[dict, bool]:
    """Use only the centralized rider-level financial reporting rows."""
    rows, finalized = courier_financial_rows(db, tenant_id, month)
    return {
        row["courier_id"]: row for row in rows if row.get("courier_id") in allowed_ids
    }, finalized
...
def _sum_financial(rows: list[dict]) -> dict:
    ...
    total["cost_per_order"] = (
        round(total["payroll_cost"] / total["eligible_orders"], 2)
        if total["eligible_orders"]
        else 0
    )
    total["revenue_per_order"] = (
        round(total["client_revenue"] / total["eligible_orders"], 2)
        if total["eligible_orders"]
        else 0
    )
    total["operational_margin_pct"] = (
        round(total["operational_margin"] / total["client_revenue"] * 100, 2)
        if total["client_revenue"]
        else 0
    )
    return total
...
def report_filter_options(
    ...
    geo = (
        {
            row.id: row.name
            for row in db.query(GeoCity)
            .filter(GeoCity.id.in_({item.geo_city_id for item in operating}))
            .all()
        }
        if operating
        else {}
    ...
    if isinstance(effective_on, str):
        effective_on = date.fromisoformat(effective_on)
    scoped_ids = {row.id for row in all_scoped}
    memberships = (
        db.query(TeamMembership)
        .filter(
            TeamMembership.tenant_id == user.tenant_id,
            TeamMembership.courier_id.in_(scoped_ids),
    ...
    membership_by_courier = {row.courier_id: row for row in memberships}
    teams = {
        row.id: row
        for row in db.query(WorkforceTeam)
        .filter(
            WorkforceTeam.tenant_id == user.tenant_id,
            WorkforceTeam.is_active.is_(True),
        )
        .all()
    }
    zones = {
        row.id: row
        for row in db.query(OperatingZone)
        .filter(
            OperatingZone.tenant_id == user.tenant_id,
            OperatingZone.is_active.is_(True),
        )
        .all()
    }
    return {
        "cities": [
            {
                "id": item.geo_city_id,
                "name": item.display_name or geo.get(item.geo_city_id, "—"),
            }
            for item in sorted(
                operating,
                key=lambda row: (
                    row.display_name or geo.get(row.geo_city_id, ""),
                    row.id,
                ),
            )
        ],
        "contracts": [
            ...
            for row in sorted(
                (row for row in contracts.values() if row.id in contract_ids),
        ...
        "branches": [
            ...
            for row in sorted(branches.values(), key=lambda row: (row.city, row.id))
        ],
        "projects": [
            {"id": key, "name": value}
            for key, value in sorted(projects.items(), key=lambda item: item[1])
        ],
        "supervisors": [
            {"id": key, "name": value}
            for key, value in sorted(supervisors.items(), key=lambda item: item[1])
        ],
        "zones": [
            {"id": row.id, "name": row.name_ar, "name_en": row.name_en}
            for row in sorted(zones.values(), key=lambda row: (row.name_ar, row.id))
        ],
        "teams": [
            {
                "id": row.id,
            ...
            for row in sorted(teams.values(), key=lambda row: (row.name_ar, row.id))
        ],
        "riders": [
            {
                "id": row.id,
                "name": row.name,
                "city_id": row.city_id,
                "branch_id": row.contract_branch_id,
                "project_id": row.primary_project_id,
                "supervisor_id": row.supervisor_id,
                "team_id": membership_by_courier[row.id].team_id
                if row.id in membership_by_courier
                else None,
                "zone_id": teams[membership_by_courier[row.id].team_id].zone_id
                if row.id in membership_by_courier
                and membership_by_courier[row.id].team_id in teams
                else None,
            }
            for row in sorted(selected, key=lambda row: (row.name, row.id))[:200]
        ],
...
def analytics_report(
    ...
    allow_payroll = "*" in permissions or (
        "payroll" in permissions and user.role != UserRole.SUPERVISOR
    )
    if report_type == "financial" and not allow_commercial:
        raise PermissionError(
            "Commercial financial reporting is restricted to company accounts"
        )
    if report_type == "executive" and not allow_commercial:
        raise PermissionError(
            "Executive commercial reporting is restricted to company accounts"
        )

    query = _apply_filters(
        _scoped_courier_query(db, user, supervisor_scope), db, user.tenant_id, filters
    )
    ...
    financial_by_courier, financial_finalized = (
        _financial_maps(db, user.tenant_id, month, set(ids))
        if (allow_payroll or allow_commercial)
        else ({}, False)
    )
    ...
    for courier in couriers:
        ...
        row = {
            ...
            "city": names["cities"].get(courier.city_id)
            or (branch.city if branch else courier.work_city or "—"),
            "contract_id": branch.contract_id if branch else None,
            "contract": names["contracts"].get(branch.contract_id).name
            if branch and names["contracts"].get(branch.contract_id)
            else "—",
            "project_id": courier.primary_project_id,
            "project": names["projects"].get(
                courier.primary_project_id, courier.platform or "—"
            ),
            "branch_id": courier.contract_branch_id,
            "branch": branch.city if branch else "—",
            "supervisor_id": courier.supervisor_id,
            "supervisor": names["supervisors"].get(courier.supervisor_id, "—"),
            "eligible_orders": current_orders,
            "mtd_orders": mtd_orders,
            "target": target,
            "achievement_pct": round(mtd_orders / target * 100, 2) if target else None,
            "worked_hours": worked_hours,
            "orders_per_worked_hour": round(current_orders / worked_hours, 2)
            if worked_hours
            else 0,
            ...
            "segment": "SUSPENDED"
            if (courier.employment_status or "ACTIVE") != "ACTIVE"
            else (
                "ZERO_ORDERS"
                if current_orders == 0
                else ("TARGET_ACHIEVED" if bonus.get("achieved") else "BELOW_TARGET")
            ),
        }
        if allow_payroll:
            row.update(
                {
                    ...
                    "cost_per_order": round(
                        float(fin.get("payroll_cost") or 0)
                        / int(fin.get("eligible_orders") or 0),
                        2,
                    )
                    if int(fin.get("eligible_orders") or 0)
                    else 0,
                }
            )
            ...
            row.update(
                {
                    ...
                    "operational_margin": round(
                        float(fin.get("operational_margin") or 0), 2
                    ),
                    "revenue_per_order": round(
                        float(fin.get("client_revenue") or 0)
                        / int(fin.get("eligible_orders") or 0),
                        2,
                    )
                    if int(fin.get("eligible_orders") or 0)
    ...
    base_kpis = {
        "total_riders": len(detail),
        "active_riders": len(active),
        "online_riders": sum(row["online"] for row in detail),
        "suspended_riders": sum(
            row["employment_status"] == "SUSPENDED" for row in detail
        ),
        "eligible_orders": total_orders,
        "orders_per_active_rider": round(total_orders / len(active), 2)
        if active
        else 0,
        "worked_hours": round(total_hours, 2),
        "orders_per_worked_hour": round(total_orders / total_hours, 2)
        if total_hours
        else 0,
        "attendance_rate": round(attendance_days / (len(active) * day_count) * 100, 2)
        if active
        else 0,
        ...
        "target_achievement_pct": round(
            sum(row["mtd_orders"] for row in target_rows)
            / sum(row["target"] for row in target_rows)
            * 100,
            2,
        )
        if target_rows and sum(row["target"] for row in target_rows)
        else 0,
    ...
    if include_commercial:
        totals = _sum_financial(
            [
                financial_by_courier[row["rider_id"]]
                for row in detail
                if row["rider_id"] in financial_by_courier
            ]
        )
        base_kpis.update(
            {
                "total_bonus_cost": totals["bonus"],
                "estimated_or_final_payroll_cost": totals["payroll_cost"],
                "client_revenue": totals["client_revenue"],
                "operational_margin": totals["operational_margin"],
                "operational_margin_pct": totals["operational_margin_pct"],
                "financial_finalized": financial_finalized,
            }
        )
    elif allow_payroll:
        base_kpis["estimated_or_final_payroll_cost"] = round(
            sum(row.get("payroll_cost") or 0 for row in detail), 2
        )

    city_orders, project_orders, supervisor_orders = (
        defaultdict(int),
        defaultdict(int),
        defaultdict(int),
    )
    city_financial, project_financial = (
        defaultdict(lambda: {"revenue": 0.0, "cost": 0.0, "margin": 0.0}),
        defaultdict(lambda: {"revenue": 0.0, "cost": 0.0, "margin": 0.0}),
    )
    for row in detail:
        ...
        if fin and include_commercial:
            for bucket, key in (
                (city_financial, row["city"]),
                (project_financial, row["project"]),
            ):
    ...
    def ranked(mapping, key_name, limit=8):
        return [
            {key_name: key, "eligible_orders": value}
    ...
    charts = {
        "orders_trend": [
            {"date": key, "eligible_orders": value}
            for key, value in sorted(activity["orders_by_day"].items())
        ],
        ...
        "top_riders": [
            ...
            for row in sorted(
                detail, key=lambda row: (-row["eligible_orders"], row["rider"])
            )[:10]
        ],
    }
    if include_commercial:
        charts["financial_by_project"] = [
            {
                "project": key,
                "client_revenue": round(value["revenue"], 2),
                "rider_cost": round(value["cost"], 2),
                "operational_margin": round(value["margin"], 2),
            }
            for key, value in sorted(
                project_financial.items(), key=lambda item: -item[1]["margin"]
            )[:8]
        ]
        charts["revenue_by_city"] = [
            {"city": key, "client_revenue": round(value["revenue"], 2)}
            for key, value in sorted(
                city_financial.items(), key=lambda item: -item[1]["revenue"]
            )[:8]
        ]
    ...
    sort_key = (
        (lambda row: (-row["eligible_orders"], row["rider"]))
        if report_type != "workforce"
        else (
            lambda row: (
                row["segment"] != "ZERO_ORDERS",
                -row["eligible_orders"],
                row["rider"],
            )
        )
    ...
    return {
        ...
        "pagination": {
            ...
            "page": page,
            "page_size": page_size,
            "pages": (len(detail) + page_size - 1) // page_size,
        },
        "definitions": {
...
def flat_export_rows(
    report: dict, include_commercial: bool, include_payroll: bool
) -> list[dict]:
