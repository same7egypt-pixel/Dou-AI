...
def _permissions_for(user: User):
    return _json_list(user.custom_permissions, ROLE_PERMISSIONS.get(user.role, []))
...
def _require_legacy_delivery():
    if not ENABLE_LEGACY_DELIVERY:
        raise HTTPException(404, "Legacy delivery feature is disabled")
...
def _tenant_record(db: Session, model, record_id: int, user: User):
    ...
    if (
        ...
        and getattr(record, "tenant_id", None) != user.tenant_id
    ):
        raise HTTPException(404, f"{model.__name__} not found")
    return record
...
def _report_rows(
    ...
    if user.role == UserRole.SUPERVISOR:
        ...
        q = q.filter(
            Courier.primary_project_id.in_(_json_list(user.managed_project_ids))
        )
    if contract_id:
        contract_projects = db.query(ContractBranch.project_id).filter(
            ContractBranch.contract_id == contract_id
        )
        q = q.filter(
            or_(
                Courier.contract_id == contract_id,
    ...
    if branch_id:
        ...
        q = (
            q.filter(
                or_(
                    Courier.contract_branch_id == branch_id,
    ...
    if report_type == "documents":
        ...
        for c in couriers:
            ...
            status = (
                "EXPIRED"
                if any(v is not None and v < 0 for v in days)
                else ("SOON" if any(v is not None and v <= 30 for v in days) else "OK")
            )
            ...
            rows.append(
                {
                    "السائق": c.name,
                    "الجوال": c.phone,
                    "المشروع": c.platform or "—",
                    "نوع المركبة": c.vehicle_type or "—",
                    "رقم اللوحة": c.vehicle_plate or "—",
                    "انتهاء الإقامة": c.iqama_expiry.isoformat()
                    if c.iqama_expiry
                    else "—",
                    "انتهاء رخصة القيادة": c.license_expiry.isoformat()
                    if c.license_expiry
                    else "—",
                    "انتهاء رخصة المركبة": c.vehicle_license_expiry.isoformat()
    ...
    for c in couriers:
        ...
        rows.append(
            {
                "السائق": c.name,
                "المشروع": project.name if project else c.platform or "—",
                "طلبات الفترة": period_orders,
                "طلبات الشهر حتى نهاية الفترة": bonus["orders"],
                "التارجت الشهري": bonus["target"],
                "المتبقي": bonus["remaining_orders"],
                "الطلبات الزائدة": bonus["over_orders"],
                "بونص التارجت": bonus["bonus_amount"],
                "سعر الطلب الزائد": bonus["over_target_rate"],
...
def analytics_view(
    ...
    start = date_from or date(end.year, end.month, 1)
    filters = _analytics_filters(
        city_id,
        contract_id,
        branch_id,
...
def analytics_export(
    ...
    payroll = "*" in permissions or (
        "payroll" in permissions and user.role != UserRole.SUPERVISOR
    )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="dou-{report_type}-{start}-{end}.csv"'
        },
    )
...
@router.get("/reports")
def fleet_reports(
    report_type: str = Query("documents", pattern="^(documents|bonus)$"),
    project_id: int = None,
    contract_id: int = None,
    branch_id: int = None,
    supervisor_id: int = None,
...
def _courier_ids(db: Session, tenant_id, supervisor_id=None, project_ids=None):
    ...
    if project_ids is not None:
        q = q.filter(Courier.primary_project_id.in_(project_ids))
...
def _supervisor_can_access_courier(
    db: Session, supervisor_id: int, courier: Courier
) -> bool:
    return (
        db.query(Courier.id)
        .filter(
            Courier.id == courier.id,
            supervisor_courier_scope(db, supervisor_id),
        )
        .first()
        is not None
    )
...
def fleet_me(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if user.role not in TENANT_ROLES and user.role not in (
        UserRole.DOU_ADMIN,
        UserRole.DOU_OPS,
    ):
        raise HTTPException(403, "Not a fleet account")
    ...
    return [
        {
            ...
            "custom_permissions": _json_list(row.custom_permissions),
            "managed_project_ids": _json_list(row.managed_project_ids),
        }
        for row in rows
    ]
...
def fleet_add_user(
    ...
    row = User(
        ...
        is_active=True,
        custom_permissions=json.dumps(payload.get("permissions"))
        if payload.get("permissions")
        else None,
        managed_project_ids=json.dumps(payload.get("project_ids") or []),
    )
    db.add(row)
    db.commit()
    db.refresh(row)
...
def fleet_update_user(
    ...
    if "is_active" in payload:
        if user.role not in ACCOUNT_ADMIN_ROLES:
            raise HTTPException(
                403, "Only the company admin can activate or deactivate accounts"
            )
    ...
    if "permissions" in payload:
        row.custom_permissions = json.dumps(payload.get("permissions") or [])
    if "project_ids" in payload:
        row.managed_project_ids = json.dumps(payload.get("project_ids") or [])
    db.commit()
    return {"ok": True}


@router.delete("/users/{uid}")
def fleet_delete_user(
    uid: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    if user.role not in ACCOUNT_ADMIN_ROLES:
...
def fleet_overview(
    ...
    tenant_id = _scope(user, db)
    managed = (
        _json_list(user.managed_project_ids)
    ...
    for courier in couriers:
        dates = (
            courier.iqama_expiry,
            courier.license_expiry,
            courier.vehicle_license_expiry,
        )
    ...
    active_employees = sum(
        1 for c in couriers if (c.employment_status or "ACTIVE") == "ACTIVE"
    )
    ...
    financial_preview, financial_finalized = (
        financial_rows(db, tenant_id, selected_month, payroll_preview)
        if tenant_id is not None and user.role != UserRole.SUPERVISOR
        else ([], False)
    )
    ...
    return {
        ...
        "documents_30": docs_30,
        "documents_60": docs_60,
        "present_today": len({a.courier_id for a in today_att}),
        "late_today": sum(bool(a.is_late) for a in today_att),
        "absent_today": max(
            active_employees - len({a.courier_id for a in today_att}) - on_leave, 0
        ),
        "orders_today": sum(x.orders_count or 0 for x in logs if x.log_date == today),
        "orders_month": sum(x.orders_count or 0 for x in logs),
        "pending_leaves": db.query(LeaveRequest)
        .filter(
            LeaveRequest.courier_id.in_(ids),
            LeaveRequest.status.in_(["PENDING", "SUPERVISOR_APPROVED"]),
        )
        .count()
        if ids
        else 0,
        ...
        "payroll_total": round(
            sum(float(row.get("net_pay") or 0) for row in payroll_preview), 2
        ),
        "payroll_finalized": payroll_finalized,
        "client_revenue": round(
            sum(float(row.get("client_revenue") or 0) for row in financial_preview), 2
        ),
        "operational_margin": round(
            sum(float(row.get("operational_margin") or 0) for row in financial_preview),
            2,
        ),
        ...
        "avg_acceptance": round(
            sum(c.acceptance_rate for c in couriers) / len(couriers), 1
        )
        if couriers
        else 0,
        "avg_score": round(sum(c.score for c in couriers) / len(couriers), 2)
        if couriers
        else 0,
        "on_time_rate": round(sum(c.on_time_rate for c in couriers) / len(couriers), 1)
        if couriers
        else 0,
        "company_couriers": sum(
            1 for c in couriers if c.courier_type.value == "COMPANY"
        ),
        "freelance_couriers": sum(
            1 for c in couriers if c.courier_type.value == "FREELANCER"
        ),
        "not_ready": db.query(OperationalReadinessState)
        .filter(
            OperationalReadinessState.tenant_id == tenant_id,
            OperationalReadinessState.overall_status != "READY",
        )
        .count()
        if tenant_id
        else 0,
        "shifts_active": (
            sum(1 for c in couriers if c.shift_active)
...
def fleet_needs_attention(
    ...
    events = (
        ...
        .filter(
            ...
            AttendanceEvent.courier_id.in_(ids),
    ...
    add(
        "ABSENT",
        "مندوبون غائبون بعد تسوية الحضور",
        sum(event.event_type == "ABSENCE" for event in events),
        "attendance",
        "high",
    )
    add(
        "LATE",
        "مندوبون متأخرون اليوم",
        sum(event.event_type == "LATE" for event in events),
        "attendance",
        "medium",
    )
    ...
    pending_events = (
        ...
        .filter(
            AttendanceEvent.tenant_id == tenant_id,
            AttendanceEvent.courier_id.in_(ids),
            AttendanceEvent.status == "PENDING_APPROVAL",
        )
        .count()
    )
    add(
        "DEDUCTION_APPROVALS",
        "خصومات حضور بانتظار الاعتماد",
        pending_events,
        "attendance-events",
        "medium",
    )
    if user.role != UserRole.SUPERVISOR:
        failed_imports = (
            ...
            .filter(
                OperationalImportBatch.tenant_id == tenant_id,
                OperationalImportBatch.status == "PREVIEW",
                OperationalImportBatch.invalid_rows > 0,
            )
            .count()
        )
        add(
            "IMPORT_ERRORS",
            "دفعات استيراد تتطلب تصحيحاً",
            failed_imports,
            "imports",
            "medium",
        )
        ...
        add(
            "PAYROLL_REVIEW",
            "رواتب الشهر في المعاينة ولم تُقفل",
            1 if not period or period.status != "FINALIZED" else 0,
            "payroll",
            "low",
        )
    return {
...
def fleet_couriers(
    ...
    branch_ids = {
        courier.contract_branch_id for courier in couriers if courier.contract_branch_id
    }
    fleet_ids = {courier.fleet_id for courier in couriers if courier.fleet_id}
    supervisor_ids = {
        courier.supervisor_id for courier in couriers if courier.supervisor_id
    }
    project_ids = {
        courier.primary_project_id for courier in couriers if courier.primary_project_id
    }
    contracts = (
        {
            row.id: row.name
            for row in db.query(Contract)
            .filter(Contract.tenant_id == tenant_id, Contract.id.in_(contract_ids))
            .all()
        }
        if contract_ids
        else {}
    ...
    branches = (
        {
            ...
            .filter(
                ContractBranch.tenant_id == tenant_id, ContractBranch.id.in_(branch_ids)
            )
            .all()
        }
        if branch_ids
    ...
    fleets = (
        {
            row.id: row.name
            for row in db.query(Fleet)
            .filter(Fleet.tenant_id == tenant_id, Fleet.id.in_(fleet_ids))
            .all()
        }
        if fleet_ids
        else {}
    ...
    supervisors = (
        {
            ...
            .filter(User.tenant_id == tenant_id, User.id.in_(supervisor_ids))
            .all()
        }
        if supervisor_ids
        else {}
    ...
    projects = (
        {
            ...
            .filter(Project.tenant_id == tenant_id, Project.id.in_(project_ids))
            .all()
        }
        if project_ids
        else {}
    ...
    courier_accounts = (
        {
            ...
            for row in db.query(User)
            .filter(User.courier_id.in_(ids), User.role == UserRole.COURIER)
            .all()
        }
        if ids
        else {}
    ...
    return [
        {
            ...
            "score": c.score,
            "documents_valid": c.documents_valid,
            "shift_active": c.shift_active,
            "lat": c.lat,
            "lng": c.lng,
            "base_salary": c.base_salary or 0,
            "per_delivery_rate": c.per_delivery_rate or 0,
            "bonus_target": c.bonus_target or 0,
...
def paged_couriers(
    ...
    if city_id:
        q = q.filter(Courier.city_id == city_id)
    if branch_id:
        q = q.filter(Courier.contract_branch_id == branch_id)
    if supervisor_id:
        q = q.filter(Courier.supervisor_id == supervisor_id)
    if project_id:
        q = q.filter(Courier.primary_project_id == project_id)
    if operator_id:
        from app.models.entities import RiderAssignment

        assigned_cids = [
            row[0]
            for row in db.query(RiderAssignment.courier_id)
            .filter(
                RiderAssignment.tenant_id == tenant_id,
                RiderAssignment.operator_id == operator_id,
    ...
    if online is not None:
        q = q.filter(Courier.is_online.is_(online))
    if documents_valid is not None:
        q = q.filter(Courier.documents_valid.is_(documents_valid))
    total = q.count()
    page = max(1, page)
    page_size = min(max(1, page_size), 200)
    rows = (
        q.order_by(Courier.name, Courier.id)
    ...
    branch_map = {
        ...
        for row in db.query(ContractBranch)
        .filter(
            ContractBranch.id.in_(
                [c.contract_branch_id for c in rows if c.contract_branch_id]
            )
        )
        .all()
    }
    supervisor_map = {
        row.id: row.name
        for row in db.query(User)
        .filter(User.id.in_([c.supervisor_id for c in rows if c.supervisor_id]))
        .all()
    }
    project_map = {
        row.id: row.name
        for row in db.query(Project)
        .filter(
            Project.id.in_([c.primary_project_id for c in rows if c.primary_project_id])
        )
        .all()
    }
    contract_map = {
        row.id: row.name
        for row in db.query(Contract)
        .filter(Contract.id.in_([c.contract_id for c in rows if c.contract_id]))
        .all()
    }
    today = date.today()
    month_start = date(today.year, today.month, 1)
    ...
    if rows:
        for log in (
            db.query(DailyLog)
            .filter(
                DailyLog.courier_id.in_([c.id for c in rows]),
                DailyLog.log_date >= month_start,
                DailyLog.log_date <= today,
            )
            .all()
        ...
        "rows": [
            {
                ...
                "contract_branch_id": c.contract_branch_id,
                "contract_id": c.contract_id,
                "contract": contract_map.get(c.contract_id),
                "branch": branch_map.get(c.contract_branch_id).city
                if c.contract_branch_id in branch_map
                else None,
                "supervisor_id": c.supervisor_id,
                "supervisor": supervisor_map.get(c.supervisor_id),
                "project_id": c.primary_project_id,
                "project": project_map.get(c.primary_project_id),
                "today_orders": today_orders[c.id],
                "month_orders": month_orders[c.id],
            }
            for c in rows
...
def rider_import_template(user: User = Depends(get_current_user)):
    ...
    _require_permission(user, "drivers")
    return StreamingResponse(
        iter([rider_template_csv()]),
        media_type="text/csv; charset=utf-8",
        headers={
            "Content-Disposition": "attachment; filename=rider-import-template.csv"
        },
    )


@router.post("/imports/riders/preview")
def preview_riders_import(
    payload: dict, user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    if user.role not in COMPANY_ROLES:
        raise HTTPException(403, "Company rider-import access required")
    ...
    _audit(
        db,
        user,
        f"عاين استيراد مندوبي الدفعة #{result['id']}",
        "rider_import",
        result["id"],
    )
    db.commit()
...
def confirm_riders_import(
    ...
    _audit(
        db,
        user,
        f"أكد استيراد {result.get('result', {}).get('imported', 0)} مندوب",
        "rider_import",
        batch.id,
    )
    db.commit()
...
def performance_import_template(user: User = Depends(get_current_user)):
    ...
    return StreamingResponse(
        iter([performance_template_csv()]),
        media_type="text/csv; charset=utf-8",
        headers={
            "Content-Disposition": "attachment; filename=performance-import-template.csv"
        },
    )


@router.post("/imports/performance/preview")
def preview_performance_file(
    payload: dict, user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    if user.role not in COMPANY_ROLES:
        raise HTTPException(403, "Company performance-import access required")
    ...
    _audit(
        db,
        user,
        f"عاين استيراد أداء الدفعة #{result['id']}",
        "performance_import",
        result["id"],
    )
    db.commit()
...
def confirm_performance_file(
    ...
    if (
        ...
        or batch.import_type != "PERFORMANCE"
    ):
    ...
    _audit(
        db,
        user,
        f"أكد استيراد أداء: {result.get('result', {}).get('imported', 0)} جديد و{result.get('result', {}).get('updated', 0)} محدث",
        "performance_import",
        batch.id,
    )
    db.commit()
...
def add_courier(
    ...
    return {
        "ok": True,
        "id": courier.id,
        "login_phone": courier.phone,
        "supervisor": db.get(User, courier.supervisor_id).name
        if courier.supervisor_id
        else None,
        "project": db.get(Project, courier.primary_project_id).name
...
def update_courier(
    ...
    if "employment_status" in payload and user.role not in ACCOUNT_ADMIN_ROLES:
        raise HTTPException(
            403, "Only the company admin can activate or deactivate couriers"
        )
    for k, v in payload.items():
        if k in allowed:
            setattr(courier, k, v)
    if "national_id_or_iqama" in payload:
        courier.iqama_number = courier.national_id_or_iqama
    if "contract_id" in payload or "contract_branch_id" in payload:
        try:
            apply_branch_assignment(db, courier, payload)
        except ValueError as exc:
            raise HTTPException(400, str(exc)) from exc
    if "supervisor_id" in payload:
        supervisor_id = (
            int(payload["supervisor_id"]) if payload["supervisor_id"] else None
        )
        if supervisor_id:
            supervisor = (
                db.query(User)
    ...
    if "employment_status" in payload:
        account = (
            db.query(User)
            .filter(User.courier_id == courier.id, User.role == UserRole.COURIER)
            .first()
        )
        if account:
    ...
    db.commit()
    _audit(db, user, f"تعديل بيانات السائق {courier.name}", "courier", courier.id)
    db.commit()
    return {"ok": True}
...
def bulk_update_couriers(
    ...
    couriers = (
        db.query(Courier)
        .filter(Courier.tenant_id == user.tenant_id, Courier.id.in_(courier_ids))
        .order_by(Courier.id)
        .all()
    )
    ...
    try:
        if action == "ASSIGN_BRANCH":
            ...
            for courier in couriers:
                ...
                account = (
                    db.query(User)
                    .filter(
                        User.courier_id == courier.id, User.role == UserRole.COURIER
                    )
                    .first()
                )
                if account:
        ...
        _audit(
            db,
            user,
            f"عملية جماعية {action} على {len(changed)} مندوب",
            "courier_bulk_operation",
            None,
        )
        db.commit()
    except ValueError as exc:
...
def delete_courier(
    ...
    # Check if rider has active operational history
    has_orders = db.query(Order).filter(Order.courier_id == cid).first() is not None
    has_attendance = (
        db.query(Attendance).filter(Attendance.courier_id == cid).first() is not None
    )
    has_logs = db.query(DailyLog).filter(DailyLog.courier_id == cid).first() is not None
    has_tasks = (
        db.query(CourierTask).filter(CourierTask.courier_id == cid).first() is not None
    )

    if has_orders or has_attendance or has_logs or has_tasks:
        courier.employment_status = "INACTIVE"
        courier.is_online = False
        db.commit()
        return {"ok": True, "action": "deactivated"}

    from ..models.entities import RiderVehicleAssignment

    try:
        db.query(RiderVehicleAssignment).filter(
            RiderVehicleAssignment.courier_id == cid
        ).delete(synchronize_session=False)
    except Exception:
        pass
    db.query(User).filter(User.courier_id == cid, User.role == UserRole.COURIER).delete(
...
def courier_profile(
    ...
    _require_permission(user, "drivers")
    courier = _tenant_record(db, Courier, cid, user)
    if user.role == UserRole.SUPERVISOR and not _supervisor_can_access_courier(
        db, user.id, courier
    ):
        raise HTTPException(404, "Courier not found")
    if (
        user.role == UserRole.PROJECT_MANAGER
        and courier.primary_project_id not in _json_list(user.managed_project_ids)
    ):
        raise HTTPException(404, "Courier not found")
    ...
    daily_logs = (
        db.query(DailyLog)
        .filter(DailyLog.courier_id == cid, DailyLog.log_date >= month_start)
        .all()
    )
    ...
    return {
        ...
        "score": courier.score,
        "documents_valid": courier.documents_valid,
        "shift_active": courier.shift_active,
        "lat": courier.lat,
        "lng": courier.lng,
        "base_salary": payroll["base_salary"],
        "per_delivery_rate": per_delivery,
        "bonus_target": payroll["bonus"]["target"],
        "bonus_earned": bonus,
        "bonus_source": payroll["bonus"]["source"],
        "bonus_plan_id": payroll["bonus"]["plan_id"],
        ...
        "iqama_expiry": courier.iqama_expiry.isoformat()
        if courier.iqama_expiry
        else None,
        "license_expiry": courier.license_expiry.isoformat()
        if courier.license_expiry
        else None,
        "vehicle_license_expiry": courier.vehicle_license_expiry.isoformat()
        if courier.vehicle_license_expiry
        else None,
        "vehicle_type": courier.vehicle_type,
        "vehicle_plate": courier.vehicle_plate,
        "iqama_number": courier.iqama_number,
        "emergency_name": courier.emergency_name,
        "emergency_phone": courier.emergency_phone,
        "passport_number": courier.passport_number,
        "passport_expiry": courier.passport_expiry.isoformat()
        if courier.passport_expiry
        else None,
        "insurance_expiry": courier.insurance_expiry.isoformat()
        if courier.insurance_expiry
        else None,
        "inspection_expiry": courier.inspection_expiry.isoformat()
        if courier.inspection_expiry
        else None,
        "work_permit_expiry": courier.work_permit_expiry.isoformat()
        if courier.work_permit_expiry
        else None,
        "platform": courier.platform,
        "platform_courier_id": courier.platform_courier_id,
        "zone": courier.zone,
        "city_id": courier.city_id,
        "work_city": courier.work_city,
        "shift_preference": courier.shift_preference,
        "primary_project_id": courier.primary_project_id,
        "supervisor_id": courier.supervisor_id,
        "contract_id": courier.contract_id,
...
def fleet_payouts(
    ...
    couriers = {
        courier.id: courier
        for courier in db.query(Courier).filter(Courier.tenant_id == tenant_id).all()
    }
    rows = []
        ...
        rows.append(
            {
                ...
                "per_delivery_rate": round(
                    float(row.get("per_delivery_rate", 0) or 0), 2
                ),
                "per_delivery_earned": round(float(row["delivery_pay"] or 0), 2),
                "incentive": round(
                    float(bonus.get("earned", row.get("bonus_pay", 0)) or 0), 2
                ),
    ...
    return [
        {
            ...
            "total": o.total,
            "courier_name": couriers.get(o.courier_id),
...
def fleet_shifts(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if user.role == UserRole.SUPERVISOR:
        ...
        managed = _json_list(user.managed_project_ids)
    ...
    for shift in q.order_by(Shift.id.desc()).all():
        ...
        row["fleet"] = (
            db.get(Fleet, shift.fleet_id).name
            if shift.fleet_id and db.get(Fleet, shift.fleet_id)
            else None
        )
...
def fleet_create_shift(
    ...
    start = _parse_shift_time(start_time)
    end = _parse_shift_time(end_time)
    if start == end:
    ...
    if len(courier_ids) > required_couriers:
        raise HTTPException(
            400, "عدد المناديب المسندين لا يمكن أن يتجاوز السعة المطلوبة"
        )
    valid_ids = _courier_ids(
        db, user.tenant_id, user.id if user.role == UserRole.SUPERVISOR else None
    )
    if not courier_ids.issubset(valid_ids):
        raise HTTPException(400, "يوجد مندوب لا يتبع الشركة أو فريق المشرف")
    fleet = (
        db.query(Fleet).filter(Fleet.tenant_id == user.tenant_id).first()
        if user.tenant_id
        else None
    )
    shift = Shift(
...
def fleet_attendance(
    ...
    if courier_id is not None:
        ids = [courier_id] if courier_id in ids else []
    ...
    couriers = (
        {c.id: c.name for c in db.query(Courier).filter(Courier.id.in_(ids)).all()}
        if ids
        else {}
    )
    rows = []
    for attendance in records:
        ...
        if shift:
            scheduled_start, scheduled_end, _ = _shift_window(
                shift, attendance.check_in or start
            )
            scheduled_hours = round(
                (scheduled_end - scheduled_start).total_seconds() / 3600, 1
            )
            late_minutes = max(
                0,
                int(
                    (
                        (attendance.check_in or scheduled_start) - scheduled_start
                    ).total_seconds()
                    // 60
                ),
            )
            if attendance.check_out:
                early_leave_minutes = max(
                    0, int((scheduled_end - attendance.check_out).total_seconds() // 60)
                )
        hours = (
            round(
                (attendance.check_out - attendance.check_in).total_seconds() / 3600, 1
            )
        ...
        rows.append(
            {
                "id": attendance.id,
                "name": couriers.get(attendance.courier_id),
...
def fleet_performance(
    ...
    start_day = start_date or end_day.replace(day=1)
    ...
    q = (
        db.query(Courier).filter(Courier.id.in_(ids))
        if ids
        else db.query(Courier).filter(False)
    )
    if project_id:
        q = q.filter(Courier.primary_project_id == project_id)
    if supervisor_id:
    ...
    for c in q.order_by(Courier.name).all():
        tasks = (
            ...
            .filter(
                ...
                CourierTask.offered_at >= start,
                CourierTask.offered_at < end,
            )
            .all()
        )
        offered = len(tasks)
        accepted = sum(bool(t.accepted_at) for t in tasks)
        delivered = [
            t
            for t in tasks
            if t.status == CourierTaskStatus.DELIVERED or t.delivered_at
        ]
        on_time = [
            t
            for t in delivered
            if t.accepted_at
            and t.delivered_at
            and (t.delivered_at - t.accepted_at).total_seconds() <= 3600
        ]
        ...
        punctuality = (
            round(len(on_time) / len(delivered) * 100, 1) if delivered else None
        )
        rating = max(0, min(5, float(c.score or 0)))
        available = [x for x in (acceptance, completion, punctuality) if x is not None]
        score = (
            round(
                (
                    (acceptance or 0) * 0.3
                    + (completion or 0) * 0.3
                    + (punctuality or 0) * 0.3
                    + (rating / 5 * 100) * 0.1
                )
                / 10,
                1,
        ...
        rows.append(
            {
                ...
                "supervisor_id": c.supervisor_id,
                "supervisor": supervisor.name if supervisor else "—",
                "online": c.is_online,
                "offered": offered,
                "accepted": accepted,
    ...
    accepts = [r["acceptance"] for r in rows if r["acceptance"] is not None]
    projects = (
        db.query(Project)
    ...
    return {
        ...
        "summary": {
            ...
            "avg_acceptance": round(sum(accepts) / len(accepts), 1)
            if accepts
            else None,
...
def monthly_attendance(
    ...
    start = chosen
    end = datetime(chosen.year + (chosen.month // 12), chosen.month % 12 + 1, 1)
    ids = _courier_ids(
        db, user.tenant_id, user.id if user.role == UserRole.SUPERVISOR else None
    )
    rows = []
    for c in (
        db.query(Courier).filter(Courier.id.in_(ids)).order_by(Courier.name).all()
        if ids
        else []
    ):
        records = (
        ...
        for attendance in records:
            ...
            if not shift:
                continue
            scheduled_start, scheduled_end, _ = _shift_window(
                shift, attendance.check_in
            )
            late_minutes += max(
                0, int((attendance.check_in - scheduled_start).total_seconds() // 60)
            )
            if attendance.check_out:
                early_leave_minutes += max(
                    0, int((scheduled_end - attendance.check_out).total_seconds() // 60)
        ...
        rows.append(
            {
                "المندوب": c.name,
                "أيام الحضور": len({a.check_in.date() for a in records}),
                "مرات التأخير": sum(bool(a.is_late) for a in records),
                "دقائق التأخير": late_minutes,
                "دقائق الانصراف المبكر": early_leave_minutes,
                "ساعات العمل": round(hours, 1),
                "الحالة": c.employment_status or "ACTIVE",
            }
        )
...
def fleet_create_contract(
    ...
    project = (
        db.query(Project)
        .filter(
            Project.tenant_id == (user.tenant_id if user.tenant_id else tenant_id),
            Project.name == name,
        )
        .first()
    )
    if not project:
        project = Project(
            tenant_id=user.tenant_id if user.tenant_id else tenant_id,
            name=name,
            is_active=True,
        )
        db.add(project)
    ...
    contract = Contract(
        tenant_id=user.tenant_id
        if user.tenant_id
        else (tenant_id if tenant_id is not None else None),
...
def fleet_tickets(
    ...
    courier_q = db.query(Courier)
    if user.tenant_id is not None:
        courier_q = courier_q.filter(Courier.tenant_id == user.tenant_id)
    couriers = {c.id: c.name for c in courier_q.all()}
    return [
        {
            "id": t.id,
            ...
            "status": t.status,
            "reply": t.reply,
            "courier": couriers.get(t.courier_id),
...
def fleet_reply_ticket(
    ...
    if user.role == UserRole.SUPERVISOR and ticket.courier_id not in _courier_ids(
...
def fleet_test_order(
    ...
    if ids:
        courier = (
            db.query(Courier)
            .filter(Courier.id.in_(ids), Courier.is_online.is_(True))
            .first()
        )
        if not courier:
...
def fleet_escalate(
    ...
    if not order:
        raise HTTPException(404, "Order not found")
    order.status = (
        OrderStatus.TIMEOUT if hasattr(OrderStatus, "TIMEOUT") else OrderStatus.PLACED
    )
    ...
    for c in q.all():
        ...
        rows.append(
            [
                ...
                fixed,
                per_delivery,
                round(done * per_delivery, 2),
                bonus,
                round(fixed + done * per_delivery + bonus, 2),
                c.bank_iban or "",
            ]
        )
    buffer = io.StringIO()
